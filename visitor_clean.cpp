...
public:
  int accept(BaseVisitor *v) {
    if (Visitor<Visitable1> *p = dynamic_cast<Visitor<Visitable1> *>(v))
      return p->visit(std::static_pointer_cast<Visitable1>(shared_from_this()));
  }
};
...
public:
  int accept(BaseVisitor *v) {
    if (Visitor<Visitable2> *p = dynamic_cast<Visitor<Visitable2> *>(v))
      return p->visit(std::static_pointer_cast<Visitable2>(shared_from_this()));
  }
};
